...
gbc_sentimen.fit(X_train_dense, y_train_sentimen)

# CatBoost model
catboost_sentimen = CatBoostClassifier(verbose=0, random_state=42)

# Gunakan Pool untuk CatBoost
train_data_sentimen = Pool(X_train_dense, label=y_train_sentimen)
catboost_sentimen.fit(train_data_sentimen)
...
if y_aspek is not None:
    ...
    gbc_aspek.fit(X_train_dense, y_train_aspek)
    catboost_aspek = CatBoostClassifier(verbose=0, random_state=42)
    
    # Gunakan Pool untuk CatBoost
    train_data_aspek = Pool(X_train_dense, label=y_train_aspek)
    catboost_aspek.fit(train_data_aspek)
...
if predict_btn and input_text.strip() != "":
    ...
    pred_sentimen_cat = catboost_sentimen.predict(input_vec_dense)[0]
    pred_sentimen_gbc = gbc_sentimen.predict(input_vec_dense)[0]

    if y_aspek is not None:
        pred_aspek_cat = catboost_aspek.predict(input_vec_dense)[0]
        pred_aspek_gbc = gbc_aspek.predict(input_vec_dense)[0]
    else:
